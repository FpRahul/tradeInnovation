...
class TasksController extends Controller
{
    ...
    public function duplicateVerified(Request $request, $id)
    {
        ...
        if ($id) {
            ...
            $newExistedTaskDetails->service_detail_id = $existedTaskDetails->service_detail_id;
    ...
    public function negotiatePrice(Request $request, $id)
    {
        ...
        $paymentData = Payment::where(['task_id' => $id])->latest()->first();
        $rule  = [
            'negotiatePrice' => 'required',
        ];
        $validtor = Validator::make($request->all(), $rule);
        if ($validtor->fails()) {
            return redirect()->back()->withErrors($validtor)->withInput();
    ...
    public function clientApproval($id)
    {
        ...
        return view('tasks.tradeMark.client_approval', compact(
            'id',
            'header_title_name',
            'taskDetails',
            'leadTaskdetials',
                    ...
                    if ($existedLeaedTaskDetails->save()) {
                        ...
                        if ($newLeadTaskDeatails->save()) {
                            ...
                            if ($newNotification->save()) {
                                ...
                                $newValue = [
                                    'Status' =>  'Completed',
                                    'Client Approved On' => $verifiedDate,
                                    'Assigned To' => $existedLeaedTask->user->name,
                                ];
                                $LeadLog->old_value = Json_encode($oldValue);
                                $LeadLog->new_value = Json_encode($newValue);
                                $LeadLog->description = "Client Approval status updated";
                                if ($LeadLog->save()) {
                                    $newassignlog = new leadLog();
                                    $newassignlog->user_id = $request->assignUser ?? $existedLeaedTask->user_id;
                                    $newassignlog->lead_id = $existedLeaedTask->lead_id;
                ...
                if ($newLeadtask->save()) {
                    ...
                    if ($newLeadTaskDeatails->save()) {
                        ...
                        if ($existedLeaedTaskDetails->save()) {
                            ...
                            $newValue = [
                                'Status' =>  'Client Not Approved',
                                'Verified On' => $verifiedDate,
                                'Assigned To' => $existedLeaedTask->user->name,
                            ];
                            $LeadLog->old_value = Json_encode($oldValue);
                            $LeadLog->new_value = Json_encode($newValue);
                            $LeadLog->description = "Cient approval status updated successfull";
                    ...
                    if ($newLeadTaskDeatails->save()) {
                        ...
                        if ($newNotification->save()) {
                            ...
                            $newValue = [
                                ...
                                'Assigned To' =>  $existedLeaedTask->user->name,
                                'Application Number' =>  $newServiceDetails->application_number
                            ];
                            $LeadLog->old_value = json_encode($oldValue);
                            $LeadLog->new_value = json_encode($newValue);
                    ...
                    if ($newLeadTaskDeatails->save()) {
                        ...
                        if ($newNotification->save()) {
                            ...
                            if ($LeadLog->save()) {
                                ...
                                if ($newassignlog->save()) {
                                    $id = $newLeadtask->id;
                                    return redirect()->route('task.index')->with('success', 'Formality check status completed completed');
                                }
                            } else {
                                return redirect()->back()->with('error', 'there is something wrong during upadate logs');
                            }
                        ...
                        if ($newHearing->save()) {
                            ...
                            if ($newNotification->save()) {
                                ...
                                if ($LeadLog->save()) {
                                    ...
                                    if ($newassignlog->save()) {
                                        $id = $newLeadtask->id;
                                        return redirect()->route('task.index')->with('success', 'Hearing Date Confirm');
                                    }
                                } else {
                                    return redirect()->back()->with('error', 'there is something wrong during upadate logs');
                                }
        ...
        if ($request->client_status == 1) {
                        ...
                        if ($newLeadTaskDeatails->save()) {
                            ...
                            if ($newNotification->save()) {
                                ...
                                if ($request->publish_opposition == 0) {
                                    ...
                                    $LeadLog->old_value = json_encode($oldValue);
                                    $LeadLog->new_value = json_encode($newValue);
                                    $LeadLog->description = "Opposition marked as not recevied";
                                }
            ...
            if ($id) {
                ...
                if ($newLeadtask->save()) {
                    $existedLeaedTaskDetails->status = 1;
                    $existedLeaedTaskDetails->status_date = $verifiedDate;
                    $existedLeaedTaskDetails->comment = 'Opposition Filed';
                    if ($request->hasFile('attachment')) {
                        $folderPath = public_path('uploads/leads/' . $existedLeaedTask->lead_id);
                        if (!file_exists($folderPath)) {
                            mkdir($folderPath, 0755, true);
    ...
    public function informClientAfterPublish($id)
    {
        ...
        foreach ($taskDetails as $task) {
            ...
            $clientName = $task->lead->client_name;
            $leadId = $task->lead_id;
            $servieceDetailsID = $task->service_detail_id;
        }
        ...
        $getStage = ServiceStages::where('service_id', $serviceID)->where('id', '>', $stage_id)->first();
        $leadTaskdetials = LeadTaskDetail::find($taskDetailsId);
        $service_details = ServiceDetail::where('lead_id', $leadId)->where('service_id', $serviceID)
            ->where('id', $servieceDetailsID)->first();

        return view('tasks.tradeMark.after_publish_inform_client', compact('id', 'service_details', 'header_title_name', 'taskDetails', 'leadTaskdetials', 'users', 'getStage', 'serviceName', 'clientName'));
    }
    ...
    public function oppositionPayment($id)
    {
        ...
        $leadTaskdetials = LeadTaskDetail::find($taskDetailsId);
        return view('tasks.tradeMark.opposition_payment', compact('id', 'taskDetailsId', 'service_details',  'firstPaymentId', 'payamentDetails', 'paymentId', 'header_title_name', 'taskDetails', 'leadTaskdetials', 'users', 'getStage'));
    }
    ...
    public function oppositionNoticeDate($id)
    {
        ...
        $opponentStage = ServiceStages::where('service_id', 1)
            ->where('id', 23)
            ->first();
        $onHideSatge = ServiceStages::where('service_id', 1)
            ->where('id', 26)
        ...
        return view('tasks.tradeMark.opposition_notice_date', compact(
            'id',
            'service_details',
            'header_title_name',
            'opponentStage',
    ...
    public function oppositionCounterStatement($id)
    {
        ...
        return view('tasks.tradeMark.counter_statement', compact(
            'id',
            'header_title_name',
            'taskDetails',
            'leadTaskdetials',
    ...
    public function noticeSent($id)
    {
        ...
        return view('tasks.tradeMark.registry_sent_notice', compact(
            'id',
            'header_title_name',
            'taskDetails',
            'leadTaskdetials',
    ...
    public function applicantEvidenceSubmission($id)
    {
        ...
        $service_details = ServiceDetail::where('lead_id', $taskDetails->lead_id)->where('service_id', $taskDetails->service_id)
            ->where('id', $taskDetails->service_detail_id)->first();
        return view('tasks.tradeMark.applicant_evidence', compact('id', 'service_details', 'header_title_name', 'taskDetails', 'leadTaskdetials', 'users', 'getStage', 'onHideSatge'));
    }
                            ...
                            if ($newEvidencedetails->save()) {
                                ...
                                if ($newNotification->save()) {
                                    ...
                                    if ($request->client_status == 1) {
                                        ...
                                        $newValue = [
                                            'Status' =>  'Completed',
                                            'Evidence Recevied On' => $evidence_submit,
                                            'Assigned To' => $existedLeaedTask->user->name,
                                        ];
                                        $LeadLog->old_value = Json_encode($oldValue);
                                        $LeadLog->new_value = Json_encode($newValue);
                                        $LeadLog->description = "Opponent Evidence status marked as recevied";
                                    }
                                    if ($LeadLog->save()) {
                                        $newassignlog = new leadLog();
                                        $newassignlog->user_id = $request->assignUser ?? $existedLeaedTask->user_id;
    ...
    public function clientApprovalOnHearing($id)
    {
        ...
        return view('tasks.tradeMark.client_approval_hearing', compact(
            'id',
            'header_title_name',
            'taskDetails',
            'leadTaskdetials',
    ...
    public function rectification($id){
        ...
        $stageId = $taskDetails->service_stage_id;
        $getStage = ServiceStages::where('service_id', 1)->where('id', '>', $stageId)->first();
        $onHideSatge = ServiceStages::where('service_id', 1)->where('id', '=', 53)->first();
        $leadTaskdetials = LeadTaskDetail::find($id);
        $header_title_name = $taskDetails->serviceSatge->title;
        $service_details = ServiceDetail::where('lead_id', $taskDetails->lead_id)->where('service_id', $taskDetails->service_id)
            ->where('id', $taskDetails->service_detail_id)->first();
        return view('tasks.tradeMark.rectification', compact('id', 'header_title_name', 'taskDetails', 'leadTaskdetials', 'users', 'getStage', 'onHideSatge', 'service_details'));
    }
        ...
        if ($request->client_status == 1) {
                        ...
                        if ($newLeadTaskDeatails->save()) {
                            ...
                            if ($newNotification->save()) {
                                ...
                                if ($request->publish_opposition == 0) {
                                    ...
                                    $LeadLog->remark = "Rectification Filed";
                                    $oldValue = [
                                        'status' => 'Pending',
                                        'Assigned On' => $formattedCreatedDate,
                                        'Assigned By' =>  $existedLeaedTask->userAssignBy->name,
                                    ...
                                    $newValue = [
                                        ...
                                        'Opponent Address' => $request->opponent_address,
                                        'Advocate Name' => $request->advocate_name,
                                        'Rectification Date' => $opposition_date ?? "N/A"
                                    ];
                                    $LeadLog->description = "Rectification marked as filed";
                                    $LeadLog->old_value = json_encode($oldValue);
                                    $LeadLog->new_value = json_encode($newValue);
                                } else {
                                    $LeadLog->user_id = $existedLeaedTask->user_id;
                                    ...
                                    $LeadLog->remark = "Rectification Not Filed";
                                    $oldValue = [
                                        'status' => 'Pending',
                                        'Assigned On' => $formattedCreatedDate,
                                        'Assigned By' =>  $existedLeaedTask->userAssignBy->name,
                                    ...
                                    $LeadLog->description = "Rectification marked as recevied";
                                    $LeadLog->old_value = json_encode($oldValue);
                                    $LeadLog->new_value = json_encode($newValue);
                                    $LeadLog->description = "Rectification marked as not recevied";
                                }
                                ...
                                if ($LeadLog->save()) {
                                    ...
                                    if ($newassignlog->save()) {
                                        $id = $newLeadtask->id;
                                        return redirect()->route('task.index')->with('success', 'Rectification status Updqted');
                                    }
                                } else {
                                    return redirect()->back()->with('error', 'there is something wrong during upadate logs');
                                }
            ...
            if ($id) {
                ...
                if ($newLeadtask->save()) {
                    $existedLeaedTaskDetails->status = 1;
                    $existedLeaedTaskDetails->status_date = $verifiedDate;
                    $existedLeaedTaskDetails->comment = 'Rectification Filed';
                    if ($request->hasFile('attachment')) {
                        $folderPath = public_path('uploads/leads/' . $existedLeaedTask->lead_id);
                        if (!file_exists($folderPath)) {
                            mkdir($folderPath, 0755, true);
                    ...
                    if ($existedLeaedTaskDetails->save()) {
                        ...
                        if ($newLeadTaskDeatails->save()) {
                            ...
                            if ($newNotification->save()) {
                                ...
                                $newValue = [
                                    'status' => 'Completed',
                                    'Rectification Date' => $opposition_date,
                                    'Assigned To' =>  $existedLeaedTask->user->name,
                                ];
                                $LeadLog->old_value = json_encode($oldValue);
                                $LeadLog->new_value = json_encode($newValue);
    ...
    public function informClientRectification($id){
        if ($id) {
            $notifyData = LeadNotification::where('task_id', $id)->update(['status' => 1]);
        }
        $taskDetails = LeadTask::with(['user', 'lead', 'leadTaskDetails', 'services', 'subService', 'serviceSatge'])
        ...
        return view('tasks.tradeMark.inform_client_rectification', compact('id', 'service_details', 'header_title_name', 'taskDetails', 'leadTaskdetials', 'users', 'getStage', 'serviceName', 'clientName'));
    }
     public function informClientRectificationStatus(Request $request , $id){
         $verifiedDate = Carbon::createFromFormat('d M Y', $request->input('verified'))->format('Y-m-d');
         $deadlineDate = Carbon::createFromFormat('d M Y', $request->input('deadline'))->format('Y-m-d');
         $existedTask = LeadTask::with(['services', 'subService', 'lead', 'serviceSatge'])->where('id', $id)->first();
         $existedTaskDetails = LeadTaskDetail::where('task_id', $id)->first();
                     ...
                     if ($existedTaskDetails->save()) {
                         ...
                         if ($newPayment->save()) {
                             ...
                             if ($notification->save()) {
                                 ...
                                 $LeadLog->task_id =  $existedTask->id;
                                 $LeadLog->assign_by = Auth::id();
                                 $LeadLog->remark = "Quotation sent(Rectification)";
                                 $oldValue = [
                                     'Status' => 'Pending',
                                     'Assigned On' => $formattedCreatedDate,
                                     'Assigned By' => $existedTask->userAssignBy->name,
     ...
     public function paymentOnRectification(Request $request , $id){
        if ($id) {
            $notifyData = LeadNotification::where('task_id', $id)->update(['status' => 1]);
        }
        $header_title_name = "payment Status";
        ...
        $leadTaskdetials = LeadTaskDetail::find($taskDetailsId);
        return view('tasks.tradeMark.rectification_payment', compact('id', 'taskDetailsId', 'service_details',  'firstPaymentId', 'payamentDetails', 'paymentId', 'header_title_name', 'taskDetails', 'leadTaskdetials', 'users', 'getStage'));
     }

     public function paymentOnRectificationStatus(Request $request , $id){
        // dd($request->all());
        $verifiedDate = Carbon::createFromFormat('d M Y', $request->input('verified'))->format('Y-m-d');
        if ($request->paymentDeadline) {
     ...
     public function rectificationNoticeReceived (Request $request , $id){
        if ($id) {
            LeadNotification::where('task_id', $id)->update(['status' => 1]);
        }
        ...
        return view('tasks.tradeMark.rectification_opposition_notice_date', compact(
            'id',
            'service_details',
            'header_title_name',
            'opponentStage',
     ...
     public function rectificationNoticeReceivedStatus(Request $request , $id){
        $verifiedDate = Carbon::createFromFormat('d M Y', $request->input('verified'))->format('Y-m-d');
        $deadlineDate = Carbon::createFromFormat('d M Y', $request->input('deadline'))->format('Y-m-d');
        $existedLeaedTask = LeadTask::find($id);
        $existedLeaedTaskDetails = LeadTaskDetail::where('task_id', $id)->first();
        ...
        if ($id) {
            if ($request->client_status == 1) {
                ...
                if ($newLeadtask->save()) {
                    ...
                    $existedLeaedTaskDetails->status_date = $verifiedDate;
                    if ($request->notice_received == 1) {
                        $existedLeaedTaskDetails->comment = "Rectification Notice recevied";
                    } else if ($request->notice_received == 2) {
                        $existedLeaedTaskDetails->comment = "Non-Compliance Intimation Registration(Rectification)";
                    }
                    if ($request->hasFile('attachment')) {
                        $folderPath = public_path('uploads/leads/' . $existedLeaedTask->lead_id);
                        if (!file_exists($folderPath)) {
                        ...
                        if ($newLeadTaskDeatails->save()) {
                            ...
                            if ($newNotification->save()) {
                                ...
                                if ($request->notice_received == 1) {
                                    ...
                                    $LeadLog->description = "Opposition Notice Recived (Rectification)";
                                } else if ($request->notice_received == 2) {
                                    $LeadLog->description = "Opposition Notice Not Recived (Rectification)";
                                }
                                if ($LeadLog->save()) {
                                    $newassignlog = new leadLog();
                                    $newassignlog->user_id = $request->assignUser ?? $existedLeaedTask->user_id;
                                    ...
                                    if ($newassignlog->save()) {
                                        $id = $newLeadtask->id;
                                        return redirect()->route('task.index')->with('success', 'Rectification Notice Status Update successfully');
                                    } else {
                                        return redirect()->back()->with('error', 'there is something wrong while updatng log');
                                    }
                                } else {
     ...
     public function rectificationCounterStatement( $id){
        if ($id) {
            LeadNotification::where('task_id', $id)->update(['status' => 1]);
        }
        ...
        return view('tasks.tradeMark.rectification_counter_statement', compact(
            'id',
            'header_title_name',
            'taskDetails',
            'leadTaskdetials',
     ...
     public function rectificationCounterStatementStatus(Request $request , $id){
         // dd($request->all());
         $verifiedDate = Carbon::createFromFormat('d M Y', $request->input('verified'))->format('Y-m-d');
         $deadlineDate = Carbon::createFromFormat('d M Y', $request->input('deadline'))->format('Y-m-d');
         $existedLeaedTask = LeadTask::find($id);
                 ...
                 if ($existedLeaedTaskDetails->save()) {
                     ...
                     if ($newLeadTaskDeatails->save()) {
                         ...
                         if ($newNotification->save()) {
                             ...
                             $LeadLog->new_value = json_encode($newValue);
                             if ($request->Counter_statement == 1) {
                                 $LeadLog->description = "Counter Statement Filied (Rectification)";
                             } else if ($request->Counter_statement == 2) {
                                 $LeadLog->description = "Counter Statement Not Filied (Rectification)";
                             }
                             if ($LeadLog->save()) {
                                 $newassignlog = new leadLog();
                                 $newassignlog->user_id = $request->assignUser ?? $existedLeaedTask->user_id;
                                 ...
                                 if ($newassignlog->save()) {
                                     $id = $newLeadtask->id;
                                     return redirect()->route('task.index')->with('success', 'Rectification Counter Statement Status Update Successfully');
                                 } else {
                                     return redirect()->back()->with('error', 'there is something wrong while updatng loag');
                                 }
                             } else {
     ...
     public function rectificationNoticeSent($id){
        if ($id) {
            LeadNotification::where('task_id', $id)->update(['status' => 1]);
        }
        ...
        return view('tasks.tradeMark.rectification_registry_sent_notice', compact(
            'id',
            'header_title_name',
            'taskDetails',
            'leadTaskdetials',
     ...
     public function rectificationNoticeSentStatus(Request $request , $id){
        $verifiedDate = Carbon::createFromFormat('d M Y', $request->input('verified'))->format('Y-m-d');
        $deadlineDate = Carbon::createFromFormat('d M Y', $request->input('deadline'))->format('Y-m-d');
        $existedLeaedTask = LeadTask::find($id);
        $existedLeaedTaskDetails = LeadTaskDetail::where('task_id', $id)->first();
                ...
                if ($existedLeaedTaskDetails->save()) {
                    ...
                    if ($newLeadTaskDeatails->save()) {
                        ...
                        if ($newNotification->save()) {
                            ...
                            $LeadLog->description = "Notice Sent To Opponent (Rectification)";
                            if ($LeadLog->save()) {
                                $newassignlog = new leadLog();
                                $newassignlog->user_id = $request->assignUser ?? $existedLeaedTask->user_id;
                                $newassignlog->lead_id = $existedLeaedTask->lead_id;
     ...
     public function rectificationOpponentEvidenceSubmission($id){
        if ($id) {
            LeadNotification::where('task_id', $id)->update(['status' => 1]);
        }
        ...
        return view('tasks.tradeMark.rectification_opponent_evidence', compact(
            'id',
            'header_title_name',
            'service_details',
            'taskDetails',
     ...
     public function rectificationOpponentEvidenceSubmissionStatus(Request $request , $id){
        $verifiedDate = Carbon::createFromFormat('d M Y', $request->input('verified'))->format('Y-m-d');
        if ($request->deadline) {
     ...
     public function rectifiactionApplicationEvidenceSubmission($id){
        if ($id) {
            $notifyData = LeadNotification::where('task_id', $id)->update(['status' => 1]);
        }
        ...
        $stageId = $taskDetails->service_stage_id;
        $getStage = ServiceStages::where('service_id', 1)->where('id', '>', $stageId)->first();
        $onHideSatge = ServiceStages::where('service_id', 1)->where('id', '=', 45)->first();
        $leadTaskdetials = LeadTaskDetail::find($id);
        $header_title_name = $taskDetails->serviceSatge->title;
        $service_details = ServiceDetail::where('lead_id', $taskDetails->lead_id)->where('service_id', $taskDetails->service_id)
            ->where('id', $taskDetails->service_detail_id)->first();
        return view('tasks.tradeMark.rectification_applicant_evidence', compact('id', 'service_details', 'header_title_name', 'taskDetails', 'leadTaskdetials', 'users', 'getStage', 'onHideSatge'));
     }

     public function rectifiactionApplicationEvidenceSubmissionStatus(Request $request , $id){
        $verifiedDate = Carbon::createFromFormat('d M Y', $request->input('verified'))->format('Y-m-d');
        if ($request->deadline) {
     ...
     public function rectificationOpponentResubmisson($id){
        if ($id) {
            LeadNotification::where('task_id', $id)->update(['status' => 1]);
        }
        ...
        return view('tasks.tradeMark.rectification_opponent_evidence_resubmisson', compact(
            'id',
            'header_title_name',
            'service_details',
            'taskDetails',
     ...
     public function rectificationOpponentResubmissonStatus(Request $request , $id){
        $verifiedDate = Carbon::createFromFormat('d M Y', $request->input('verified'))->format('Y-m-d');
        if ($request->deadline) {
     ...
     public function rectificationHearing($id){
        if ($id) {
            $notifyData = LeadNotification::where('task_id', $id)->update(['status' => 1]);
        }
        ...
        $leadTaskdetials = LeadTaskDetail::find($id);
        $header_title_name = $taskDetails->serviceSatge->title;
        return view('tasks.tradeMark.rectification_opposition_hearing', compact('id', 'header_title_name', 'taskDetails', 'leadTaskdetials', 'users', 'getStage', 'applicationNumber'));
     }

     public function rectificationHearingStatus(Request $request , $id){
         $hearingDate = Carbon::createFromFormat('d M Y', $request->input('hearing_date'))->format('Y-m-d');
        $verifiedDate = Carbon::createFromFormat('d M Y', $request->input('verified'))->format('Y-m-d');
        $deadlineDate = Carbon::createFromFormat('d M Y', $request->input('deadline'))->format('Y-m-d');
        $existedLeaedTask = LeadTask::find($id);
     ...
     public function clientApprovalOnHearingRectification($id){
        if ($id) {
            LeadNotification::where('task_id', $id)->update(['status' => 1]);
        }
        ...
        return view('tasks.tradeMark.rectification_client_approval_hearing', compact(
            'id',
            'header_title_name',
            'taskDetails',
            'leadTaskdetials',
     ...
     public function clientApprovalOnHearingRectificationStatus(Request $request , $id){
     ...
     public function clientInformExtraChargeOnHearingRectification($id){
        if ($id) {
            $notifyData = LeadNotification::where('task_id', $id)->update(['status' => 1]);
        }
        $taskDetails = LeadTask::with(['user', 'lead', 'leadTaskDetails', 'services', 'subService', 'serviceSatge'])
        ...
        return view('tasks.tradeMark.rectification_proforma_invoice_on_hearing', compact('id', 'header_title_name', 'taskDetails', 'leadTaskdetials', 'users', 'getStage', 'serviceName', 'clientName'));
     }

     public function clientInformExtraChargeOnHearingRectificationStatus(Request $request , $id){
         $verifiedDate = Carbon::createFromFormat('d M Y', $request->input('verified'))->format('Y-m-d');
         $deadlineDate = Carbon::createFromFormat('d M Y', $request->input('deadline'))->format('Y-m-d');
         $existedTask = LeadTask::with(['services', 'subService', 'lead', 'serviceSatge'])->where('id', $id)->first();
         $existedTaskDetails = LeadTaskDetail::where('task_id', $id)->first();
    ...
    public function followUp($id, $serviceId, $stageId)
    {
        ...
        }else if ($id == $taskDetails->id && $serviceId == 1 && $stageId == 36) {
            return redirect()->route('task.informClientRectification', ['id' => $id]);
        }else if ($id == $taskDetails->id && $serviceId == 1 && $stageId == 37) {
            return redirect()->route('task.paymentOnRectification', ['id' => $id]);
        }else if ($id == $taskDetails->id && $serviceId == 1 && $stageId == 38) {
            return redirect()->route('task.rectificationNoticeReceived', ['id' => $id]);
        }else if ($id == $taskDetails->id && $serviceId == 1 && $stageId == 39) {
            return redirect()->route('task.rectificationCounterStatement', ['id' => $id]);
        }else if ($id == $taskDetails->id && $serviceId == 1 && $stageId == 40) {
            return redirect()->route('task.rectificationNoticeSent', ['id' => $id]);
        }else if ($id == $taskDetails->id && $serviceId == 1 && $stageId == 41) {
            return redirect()->route('task.rectificationOpponentEvidenceSubmission', ['id' => $id]);
        }else if ($id == $taskDetails->id && $serviceId == 1 && $stageId == 42) {
            return redirect()->route('task.rectifiactionApplicationEvidenceSubmission', ['id' => $id]);
        }else if ($id == $taskDetails->id && $serviceId == 1 && $stageId == 43) {
            return redirect()->route('task.rectificationOpponentResubmisson', ['id' => $id]);
        }else if ($id == $taskDetails->id && $serviceId == 1 && $stageId == 46) {
            return redirect()->route('task.rectificationHearing', ['id' => $id]);
        }else if ($id == $taskDetails->id && $serviceId == 1 && $stageId == 47) {
            return redirect()->route('task.clientApprovalOnHearingRectification', ['id' => $id]);
        }
        else if ($id == $taskDetails->id && $serviceId == 1 && $stageId == 48) {
            return redirect()->route('task.clientInformExtraChargeOnHearingRectification', ['id' => $id]);
        }
    ...
    public function patentclientapproval(Request $request, $id)
    {
        ...
        $header_title_name = $taskDetails->serviceSatge->title;
        return view('tasks/patent/client-approval', compact('id', 'header_title_name', 'taskDetails', 'leadTaskdetials', 'users', 'getStage', 'previousTask'));
    }
                ...
                if ($existedLeaedTaskDetails->save()) {
                    ...
                    if ($newLeadTaskDeatails->save()) {
                        ...
                        if ($newNotification->save()) {
                            ...
                            $LeadLog->description = " Client approval marked as correct";
                            if ($LeadLog->save()) {
                                $newassignlog = new leadLog();
                                $newassignlog->user_id = $request->assignUser ?? $existedLeaedTask->user_id;
                                $newassignlog->lead_id = $existedLeaedTask->lead_id;
    ...
    public function patentfilingProccess($id)
    {
        ...
        $previousTask = LeadTaskDetail::where('task_id', '<', $taskDetails->id)
            ->orderBy('id', 'desc')
            ->first();
        ...
        if ($id) {
            ...
            if ($newLeadtask->save()) {
                ...
                if ($existedLeaedTaskDetails->save()) {
                    ...
                    $newLeadTaskDeatails->status = 0;
                    if ($newLeadTaskDeatails->save()) {
                        $serviceDetails = ServiceDetail::where(['id' => $existedLeaedTask->service_detail_id, 'lead_id' => $existedLeaedTask->lead_id, 'service_id' => $existedLeaedTask->service_id])->first();
                        ...
                        if ($newNotification->save()) {
                            ...
                            $LeadLog->old_value = json_encode($oldValue);
                            $LeadLog->new_value = json_encode($newValue);
                            $LeadLog->description = "Filing Proccess marked as completed";
                            if ($LeadLog->save()) {
                                $newassignlog = new leadLog();
                                $newassignlog->user_id = $request->assignUser ?? $existedLeaedTask->user_id;
                                $newassignlog->lead_id = $existedLeaedTask->lead_id;
    ...
    public function patentCompleteSpecification(Request $request, $id)
    {
        if ($id) {
            $notifyData = LeadNotification::where('task_id', $id)->update(['status' => 1]);
        }
    ...
    public function patentCompletedSpecificationSubmit(Request $request, $id = null)
    {
        ...
        $rule = [
            ...
            'completeSpecificationDate' => 'required',
            'deadline' => 'required',
        ];
        $validator =  Validator::make($request->all(), $rule);
        if ($validator->fails()) {
        ...
        if ($id) {
            ...
            if ($newLeadtask->save()) {
                $existedLeaedTaskDetails->status =  1;
                $existedLeaedTaskDetails->status_date = $verifiedDate;
                $existedLeaedTaskDetails->comment = 'Complete specification marked as completed';
                if ($request->hasFile('attachment')) {
                    $folderPath = public_path('uploads/leads/' . $existedLeaedTask->lead_id);
                    if (!file_exists($folderPath)) {
                        mkdir($folderPath, 0755, true);
                ...
                if ($existedLeaedTaskDetails->save()) {
                    ...
                    if ($newLeadTaskDeatails->save()) {
                        ...
                        if ($newNotification->save()) {
                            ...
                            $LeadLog->task_id = $existedLeaedTask->id;
                            $LeadLog->assign_by = Auth::id();
                            $LeadLog->remark = 'Complete specification';
                            $oldValue = [
                                'status' => 'Pending',
                                'Assigned On' => $formattedCreatedDate,
                                'Assigned By' =>  $existedLeaedTask->userAssignBy->name,
                            ...
                            $LeadLog->old_value = json_encode($oldValue);
                            $LeadLog->new_value = json_encode($newValue);
                            $LeadLog->description = "Complete specification marked as completed";
                            if ($LeadLog->save()) {
                                $newassignlog = new leadLog();
                                $newassignlog->user_id = $request->assignUser ?? $existedLeaedTask->user_id;
                                $newassignlog->lead_id = $existedLeaedTask->lead_id;
    ...
    public function patentForm9(Request $request, $id)
    {
        if ($id) {
            $notifyData = LeadNotification::where('task_id', $id)->update(['status' => 1]);
        }
        ...
        $header_title_name = $taskDetails->serviceSatge->title;
        return view('tasks.patent.form9', compact('id', 'header_title_name', 'taskDetails', 'leadTaskdetials', 'users', 'getStage', 'previousTask', 'onNextStage'));
    }

    public function patentForm9Submit(Request $request, $id)
    {
        ...
        if ($id) {
            ...
            if ($newLeadtask->save()) {
                ...
                if ($request->status == 'yes') {
                    $existedLeaedTaskDetails->comment = 'Form 9 marked as completed with early publication status';
                } else {
                    $existedLeaedTaskDetails->comment = 'Form 9 marked as completed with standard publication status';
                }
                ...
                if ($existedLeaedTaskDetails->save()) {
                    ...
                    if ($newLeadTaskDeatails->save()) {
                        ...
                        if ($newNotification->save()) {
                            ...
                            $LeadLog->task_id = $existedLeaedTask->id;
                            $LeadLog->assign_by = Auth::id();
                            $LeadLog->remark = 'Complete form 9';
                            $oldValue = [
                                'status' => 'Pending',
                                'Assigned On' => $formattedCreatedDate,
                                'Assigned By' =>  $existedLeaedTask->userAssignBy->name,
                            ...
                            $newValue = [
                                'status' => 'Completed',
                                'Filed On' => $request->verify,
                                'Assigned To' =>  $existedLeaedTask->user->name,
                            ];
                            $LeadLog->old_value = json_encode($oldValue);
                            $LeadLog->new_value = json_encode($newValue);
                            if ($request->status == 'yes') {
                                $LeadLog->description = 'Form 9 marked as completed with early publication status';
                            } else {
                                $LeadLog->description = 'Form 9 marked as completed with standard publication status';
                            }
                            if ($LeadLog->save()) {
                                $newassignlog = new leadLog();
                                $newassignlog->user_id = $request->assignUser ?? $existedLeaedTask->user_id;
    ...
    public function patentEarlyPublication(Request $request, $id)
    {
        if ($id) {
            $notifyData = LeadNotification::where('task_id', $id)->update(['status' => 1]);
        }
        ...
        $header_title_name = $taskDetails->serviceSatge->title;
        return view('tasks.patent.early-publication', compact('id', 'header_title_name', 'taskDetails', 'leadTaskdetials', 'users', 'getStage', 'previousTask', 'onNextStage'));
    }

    public function patentEarlyPublicationSubmit(Request $request, $id)
    {
        $verifiedDate = Carbon::createFromFormat('d M Y', $request->input('verify'))->format('Y-m-d');
        $deadlineDate = Carbon::createFromFormat('d M Y', $request->input('deadline'))->format('Y-m-d');
        $existedLeaedTask = LeadTask::with('services', 'lead', 'userAssignBy')->where('id', $id)->first();
        ...
        if ($id) {
            ...
            if ($newLeadtask->save()) {
                $existedLeaedTaskDetails->status =  1;
                $existedLeaedTaskDetails->status_date = $verifiedDate;
                $existedLeaedTaskDetails->comment = 'Early publication marked as completed';
                ...
                if ($existedLeaedTaskDetails->save()) {
                    ...
                    if ($newLeadTaskDeatails->save()) {
                        $serviceDetails = ServiceDetail::where(['id' => $existedLeaedTask->service_detail_id, 'lead_id' => $existedLeaedTask->lead_id, 'service_id' => $existedLeaedTask->service_id])->first();
                        $serviceDetails->journal_number = $request->journalNo;
                        $serviceDetails->save();
                        $newNotification->user_id = $request->assignUser ?? $existedLeaedTask->user_id;
                        $newNotification->lead_id = $existedLeaedTask->lead_id;
                        $newNotification->task_id = $newLeadtask->id;
                        ...
                        if ($newNotification->save()) {
                            ...
                            $LeadLog->task_id = $existedLeaedTask->id;
                            $LeadLog->assign_by = Auth::id();
                            $LeadLog->remark = 'Complete Early publication';
                            $oldValue = [
                                'status' => 'Pending',
                                'Assigned On' => $formattedCreatedDate,
                                'Assigned By' =>  $existedLeaedTask->userAssignBy->name,
                            ...
                            $LeadLog->old_value = json_encode($oldValue);
                            $LeadLog->new_value = json_encode($newValue);
                            $LeadLog->description = 'Early publication marked as completed';
                            if ($LeadLog->save()) {
                                $newassignlog = new leadLog();
                                $newassignlog->user_id = $request->assignUser ?? $existedLeaedTask->user_id;
                                $newassignlog->lead_id = $existedLeaedTask->lead_id;
                                ...
                                if ($newassignlog->save()) {
                                    $id = $newLeadtask->id;
                                    return redirect()->route('task.index')->with('success', 'Early publication completed');
                                } else {
                                    return redirect()->back()->with('error', 'there is something wrong while update logs');
                                }
                            } else {
                                return redirect()->back()->with('error', 'there is something wrong while update logs');
                            }
    ...
    public function patentStandardPublication(Request $request, $id)
    {
        if ($id) {
            $notifyData = LeadNotification::where('task_id', $id)->update(['status' => 1]);
        }
        ...
        $header_title_name = $taskDetails->serviceSatge->title;
        return view('tasks.patent.standard-publication', compact('id', 'header_title_name', 'taskDetails', 'leadTaskdetials', 'users', 'getStage', 'previousTask'));
    }

    public function patentStandardPublicationSubmit(Request $request, $id)
    {
        $verifiedDate = Carbon::createFromFormat('d M Y', $request->input('verify'))->format('Y-m-d');
        $deadlineDate = Carbon::createFromFormat('d M Y', $request->input('deadline'))->format('Y-m-d');
        $existedLeaedTask = LeadTask::with('services', 'lead', 'userAssignBy')->where('id', $id)->first();
        ...
        if ($id) {
            ...
            if ($newLeadtask->save()) {
                $existedLeaedTaskDetails->status =  1;
                $existedLeaedTaskDetails->status_date = $verifiedDate;
                $existedLeaedTaskDetails->comment = 'Standard publication marked as completed';
                ...
                if ($existedLeaedTaskDetails->save()) {
                    ...
                    if ($newLeadTaskDeatails->save()) {
                        ...
                        if ($newNotification->save()) {
                            ...
                            $LeadLog->task_id = $existedLeaedTask->id;
                            $LeadLog->assign_by = Auth::id();
                            $LeadLog->remark = 'Complete Standard publication';
                            $oldValue = [
                                'status' => 'Pending',
                                'Assigned On' => $formattedCreatedDate,
                                'Assigned By' =>  $existedLeaedTask->userAssignBy->name,
                            ...
                            $LeadLog->old_value = json_encode($oldValue);
                            $LeadLog->new_value = json_encode($newValue);
                            $LeadLog->description = 'Standard publication marked as completed';
                            if ($LeadLog->save()) {
                                $newassignlog = new leadLog();
                                $newassignlog->user_id = $request->assignUser ?? $existedLeaedTask->user_id;
                                $newassignlog->lead_id = $existedLeaedTask->lead_id;
                                ...
                                if ($newassignlog->save()) {
                                    $id = $newLeadtask->id;
                                    return redirect()->route('task.index')->with('success', 'Standard publication completed');
                                } else {
                                    return redirect()->back()->with('error', 'there is something wrong while update logs');
                                }
                            } else {
    ...
    public function patentForm18(Request $request, $id)
    {
        if ($id) {
            $notifyData = LeadNotification::where('task_id', $id)->update(['status' => 1]);
        }
    ...
    public function patentForm18Submit(Request $request, $id)
    {
        ...
        if ($id) {
            if ($request->status == 'no') {
                ...
                $existedLeaedTaskDetails->status =  2;
                $existedLeaedTaskDetails->status_date = $verifiedDate;
                $existedLeaedTaskDetails->comment = 'Form 18 marked as on hold';
                $existedLeaedTask->save();
                if ($request->hasFile('attachment')) {
                    $folderPath = public_path('uploads/leads/' . $existedLeaedTask->lead_id);
                    if (!file_exists($folderPath)) {
                ...
                if ($existedLeaedTaskDetails->save()) {
                    ...
                    $LeadLog->old_value = json_encode($oldValue);
                    $LeadLog->new_value = json_encode($newValue);
                    $LeadLog->description = 'Form 18 marked as on hold';
                    if ($LeadLog->save()) {
            ...
            if ($newLeadtask->save()) {
                $existedLeaedTaskDetails->status =  1;
                $existedLeaedTaskDetails->status_date = $verifiedDate;
                $existedLeaedTaskDetails->comment = 'Form 18 marked as completed ';
                ...
                if ($existedLeaedTaskDetails->save()) {
                    ...
                    if ($newLeadTaskDeatails->save()) {
                        ...
                        if ($newNotification->save()) {
                            ...
                            $LeadLog->task_id = $existedLeaedTask->id;
                            $LeadLog->assign_by = Auth::id();
                            $LeadLog->remark = 'Complete form 18';
                            $oldValue = [
                                'status' => 'Pending',
                                'Assigned On' => $formattedCreatedDate,
                                'Assigned By' =>  $existedLeaedTask->userAssignBy->name,
                            ...
                            $LeadLog->old_value = json_encode($oldValue);
                            $LeadLog->new_value = json_encode($newValue);
                            $LeadLog->description = 'Form 18 marked as completed';
                            ...
                            if ($LeadLog->save()) {
                                ...
                                if ($newassignlog->save()) {
                                    $id = $newLeadtask->id;
                                    return redirect()->route('task.index')->with('success', 'Form 18 marked as completed');
                                } else {
                                    return redirect()->back()->with('error', 'there is something wrong while updatng loag');
                                }
                            } else {
                ...
                if ($existedLeaedTaskDetails->save()) {
                    ...
                    if ($newLeadTaskDeatails->save()) {
                        ...
                        if ($newNotification->save()) {
                            ...
                            $LeadLog->old_value = json_encode($oldValue);
                            $LeadLog->new_value = json_encode($newValue);
                            $LeadLog->description = 'FER marked as completed';
                            if ($LeadLog->save()) {
                                $newassignlog = new leadLog();
                                $newassignlog->user_id = $request->assignUser ?? $existedLeaedTask->user_id;
                                $newassignlog->lead_id = $existedLeaedTask->lead_id;
                                ...
                                if ($newassignlog->save()) {
                                    $id = $newLeadtask->id;
                                    return redirect()->route('task.index')->with('success', 'FER successfully completed');
                                } else {
                                    return redirect()->back()->with('error', 'there is something wrong while update logs');
                                }
                            } else {
    ...
    public function patentHearingSendQuotation(Request $request, $id)
    {
        ...
        $header_title_name = "Hearing Send Quotation";
        $taskDetails = LeadTask::with(['user', 'lead','lead.LeadFirm', 'leadTaskDetails', 'services', 'subService', 'serviceSatge'])
            ->where('id', $id)
            ->get();
        foreach ($taskDetails as $task) {
        ...
        $getStage = ServiceStages::where('service_id', $serviceID)->where('id', '>', $stage_id)->first();
        $leadTaskdetials = LeadTaskDetail::find($taskDetailsId);
        return view('tasks/patent/hearing-send-quotation', compact('taskId', 'header_title_name', 'taskDetails', 'leadTaskdetials', 'users', 'getStage', 'serviceName', 'clientName'));
    }

    public function patentHearingPaymentVerification(Request $request,$id){
        $taskId = $id;
        $taskList = LeadTask::with(['user', 'leadTaskDetails', 'payment' => function ($q) {
            $q->limit(1)->latest();
        }])->where('id', $taskId)->first();
        ...
        $getStage = ServiceStages::where('service_id', 2)->where('id', '>', $taskList->service_stage_id)->first();
        $currentUser = User::find($taskList->user_id);
        $header_title_name = "Hearing Payment Verification";
        return view('tasks/patent/hearing-payment-verification', compact('header_title_name', 'firstPaymentId', 'taskId', 'getStage', 'taskList', 'serviceStage', 'userList', 'currentUser'));
    }

    public function patentawaitHearingDate(Request $request ,$id){
        if ($id) {
            $notifyData = LeadNotification::where('task_id', $id)->update(['status' => 1]);
        }
        ...
        $leadTaskdetials = LeadTaskDetail::find($id);
        $header_title_name = $taskDetails->serviceSatge->title;
        return view('tasks.patent.await-hearing-date', compact('id', 'header_title_name', 'taskDetails', 'leadTaskdetials', 'users', 'getStage'));
    }
    public function patentawaitHearingDateSubmit(Request $request,$id){
        $hearingDate = Carbon::createFromFormat('d M Y', $request->input('hearing_date'))->format('Y-m-d');
        $verifiedDate = Carbon::createFromFormat('d M Y', $request->input('verified'))->format('Y-m-d');
        $deadlineDate = Carbon::createFromFormat('d M Y', $request->input('deadline'))->format('Y-m-d');
        $existedLeaedTask = LeadTask::find($id);
    ...
    public function patentHearing(Request $request ,$id){
        if ($id) {
            $notifyData = LeadNotification::where('task_id', $id)->update(['status' => 1]);
        }
        ...
        $count = ($hearingDateCount->count ?? 0) + 1;
        return view('tasks.patent.hearing', compact('id', 'header_title_name', 'taskDetails', 'leadTaskdetials', 'users', 'getStage', 'count'));
    }

    public function patentHearingSubmit(Request $request,$id){
        $verifiedDate = Carbon::createFromFormat('d M Y', $request->input('verified'))->format('Y-m-d');
        if ($request->reminder_date) {
        ...
        if ($id) {
            ...
            if ($request->show_case_hearing == 2 || $request->show_case_hearing == 4) {
                ...
                if ($existedLeaedTaskDetails->save()) {
                    ...
                    $newNotification->task_id = $existedLeaedTask->id;
                    $newNotification->title = 'Task rejected ';
                    $newNotification->description = 'Show cause hearing  is rejected';
                    if ($newNotification->save()) {
                        $newLog =  new LeadLog();
                        $newLog->user_id = $existedLeaedTask->user_id;
                        $newLog->lead_id = $existedLeaedTask->lead_id;
                        ...
                        $newLog->old_value = json_encode($oldValue);
                        $newLog->new_value = json_encode($newValue);
                        $newLog->description = "Show cause hearing marked as rejected " . "($reason)";
                        if ($newLog->save()) {
                            return redirect()->route('task.index')->with('success', 'Show cause hearing  status is Updated');
                        } else {
                            return redirect()->back()->error('message', " there is something wrong during log generate ");
    ...
    public function patentPreGrantOpposition(Request $request,$id){
        if ($id) {
            $notifyData = LeadNotification::where('task_id', $id)->update(['status' => 1]);
        }
        ...
        $stageId = $taskDetails->service_stage_id;
        $getStage = ServiceStages::where('service_id', 2)->where('id', '>', $stageId)->first();
        $onHideSatge = ServiceStages::where('service_id', 2)->where('id', '=', 63)->first();
        $leadTaskdetials = LeadTaskDetail::find($id);
        $header_title_name = $taskDetails->serviceSatge->title;
        $service_details = ServiceDetail::where('lead_id', $taskDetails->lead_id)->where('service_id', $taskDetails->service_id)
            ->where('id', $taskDetails->service_detail_id)->first();

        return view('tasks.patent.pre-grant-opposition', compact('id', 'header_title_name', 'taskDetails', 'leadTaskdetials', 'users', 'getStage', 'onHideSatge', 'service_details'));
        
    }

    public function patentPreGrantOppositionSubmit(Request $request,$id){
        $verifiedDate = Carbon::createFromFormat('d M Y', $request->input('verified'))->format('Y-m-d');
        if ($request->opposition_date) {            
            $deadlineDate = Carbon::createFromFormat('d M Y', $request->input('deadline'))->format('Y-m-d');
        }
                        ...
                        if ($newLeadTaskDeatails->save()) {
                            ...
                            if ($newNotification->save()) {
                                ...
                                if ($LeadLog->save()) {
                                    ...
                                    if ($newassignlog->save()) {
                                        $id = $newLeadtask->id;
                                        return redirect()->route('task.index')->with('success', 'Opposition status Updated');
                                    }
                                } else {
                                    return redirect()->back()->with('error', 'there is something wrong during upadate logs');
                                }
    ...
    public function patentoppositionSendQuotation(Request $request,$id){
        $taskId = $id;
        if ($id) {
            $notifyData = LeadNotification::where('task_id', $id)->update(['status' => 1]);
        }
        $header_title_name = "Opposition Send Quotation";
        $taskDetails = LeadTask::with(['user', 'lead','lead.LeadFirm', 'leadTaskDetails', 'services', 'subService', 'serviceSatge'])
            ->where('id', $id)
            ->get();
        foreach ($taskDetails as $task) {
        ...
        $getStage = ServiceStages::where('service_id', $serviceID)->where('id', '>', $stage_id)->first();
        $leadTaskdetials = LeadTaskDetail::find($taskDetailsId);
        return view('tasks/patent/opposition-send-quotation', compact('taskId', 'header_title_name', 'taskDetails', 'leadTaskdetials', 'users', 'getStage', 'serviceName', 'clientName'));
    }

    public function patentoppositionPaymentVerification(Request $request ,$id){
        $taskId = $id;
        $taskList = LeadTask::with(['user', 'leadTaskDetails', 'payment' => function ($q) {
            $q->limit(1)->latest();
        }])->where('id', $taskId)->first();
        ...
        $getStage = ServiceStages::where('service_id', 2)->where('id', '>', $taskList->service_stage_id)->first();
        $currentUser = User::find($taskList->user_id);
        $header_title_name = "Opposition Payment Verification";
        return view('tasks/patent/opposition-payment-verification', compact('header_title_name', 'firstPaymentId', 'taskId', 'getStage', 'taskList', 'serviceStage', 'userList', 'currentUser'));
    }
