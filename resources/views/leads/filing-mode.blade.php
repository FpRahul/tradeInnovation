...
<div>
    <div class="flex items-center justify-between mb-[20px]">
        <div>
            <h3 class="text-[20px] font-[500] leading-[24px] text-[#13103A] tracking-[0.02em] mb-1.5">Manage Filing Mode</h3>
            <ul class="flex items-center text-[14px] font-[400] leading-[16px] text-[#000000] gap-[5px]">
                <li>{{$header_title_name}}</li> /
                <li class="text-gray">Filing Mode</li>
            </ul>
        </div>
        @if(in_array('filingMode.add',$permissionDetails['accessableRoutes']) || auth()->user()->role==1)
        <a href="javascript:void(0)" type="button" data-btn-name="Add"  data-name="" data-id="0" data-modal-target="assignUserModal" data-modal-toggle="assignUserModal"  class="openModalFilingMode inline-flex items-center gap-[10px] text-[13px] font-[500] leading-[15px] text-[#ffffff] tracking-[0.01em] bg-[#13103A] rounded-[10px] py-[12px] px-[30px] ">
                        ...
                        @foreach ($categoryData as $newCategorylist)
                            ...
                            <tr>
                                ...
                                <td class="border-b-[1px] border-[#0000001A] py-[12px] px-[15px]">
                                    @if((in_array('filingMode.add',$permissionDetails['accessableRoutes']) || in_array('filingMode.status',$permissionDetails['accessableRoutes'])) || auth()->user()->role==1)
                                    <div class="dropdown inline-block relative ml-[auto] mr-[20px] ">
                                        ...
                                        <div class="dropdown_menus absolute right-0 z-10 mt-2 w-[100px] origin-top-right rounded-md bg-white shadow-md ring-1 ring-black/5 focus:outline-none hidden" role="menu" aria-orientation="vertical" aria-labelledby="menu-button" tabindex="-1">
                                            <div class="text-start" role="none">
                                                @if(in_array('filingMode.add',$permissionDetails['accessableRoutes']) || auth()->user()->role==1)
                                                    <a href="javascript:void(0)" type="button" data-btn-name="edit" data-name="{{$newCategorylist->name}}" data-id="{{$newCategorylist->id}}"  class="openModalFilingMode block border-b-[1px] border-[#0000001A] hover:bg-[#f7f7f7] px-3 py-1 text-[12px] text-gray-700" data-modal-target="assignUserModal" data-modal-toggle="assignUserModal">Edit</a>
                                                @endif
                                                @if(in_array('filingMode.status',$permissionDetails['accessableRoutes']) || auth()->user()->role==1)
                                                    <a href="{{ route('filingMode.status',['id'=>$newCategorylist->id,'val' => $newCategorylist->status])}}" class="client_status block border-b-[1px] border-[#0000001A] hover:bg-[#f7f7f7] px-3 py-1 text-[12px] text-gray-700">
    ...
    <div class="relative p-4 w-full max-w-[780px] max-h-full m-auto">
        ...
        <div class="relative bg-white rounded-[20px] shadow dark:bg-gray-700">
            ...
            <div class="flex items-center justify-between p-4 md:px-5 md:py-[20px] border-b border-[#0000001A] rounded-t dark:border-gray-600">
                <h3 class="filing_add_edit_text text-[14px] font-[400] leading-[17px] text-[#000000] dark:text-white">
                   Create
                </h3>
            ...
            <!-- Modal body -->
            <div class="p-[20px]">
                <form method="POST" action={{ route('filingMode.add')}} class="space-y-[20px]">
                    @csrf
                    <input type="hidden" name="type" id="type" value="5">
                    <input type="hidden" name="filingMode_id" id="filingMode_id" value="">
                    <div class="flex flex-col md:flex-row gap-[20px]">                        
                        <div class="w-full">
                            <label for="name" class="block text-[14px] font-[400] leading-[16px] text-[#000000] mb-[5px]">Name</label>
                            <input type="text" name="name" id="filingModeName" value="" class="w-full h-[45px] border-[1px] border-[#0000001A] text-[14px] font-[400] leading-[16px] text-[#000000] tracking-[0.01em] px-[15px] py-[10px] rounded-[10px] !outline-none">
                        </div>
                    </div>
...
<script>
    $(document).on('click','.openModalFilingMode',function(){
        let type = $(this).attr('data-btn-name');
        let name = $(this).attr('data-name');
        let id = $(this).attr('data-id');
        if(id > 0){
            $('.filing_add_edit_text').text('Update');
        }else{
            $('.filing_add_edit_text').text('Create');
        }
        $('#filingMode_id').val(id);
        $('#filingModeName').val(name);
    });
