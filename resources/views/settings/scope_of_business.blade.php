...
<div>
    <div class="flex flex-col md:flex-row md:items-center justify-between mb-[20px] gap-[15px]">
        <div>
            <h3 class="text-[20px] font-[500] leading-[24px] text-[#13103A] tracking-[0.02em] mb-1.5">Manage Settings</h3>
            <ul class="flex items-center text-[14px] font-[400] leading-[16px] text-[#000000] gap-[5px]">
                <li>Settings</li> /
                <li class="text-gray">Manage Business Scope</li>
            </ul>
        </div>
        @if(in_array('incorporations.add',$permissionDetails['accessableRoutes']) || auth()->user()->role==1)
        <a href="javascript:void(0)" type="button" data-btn-name="Add"  data-name="" data-id="0" data-modal-target="assignUserModal" data-modal-toggle="assignUserModal"  class="openModalScope inline-flex items-center gap-[10px] text-[13px] font-[500] leading-[15px] text-[#ffffff] tracking-[0.01em] bg-[#13103A] rounded-[10px] py-[12px] px-[30px] ">
    ...
    <div class="shadow-[0px_0px_13px_5px_#0000000f] bg-white rounded-[20px] ">
        <div class="py-[15px] md:py-[25px] px-[15px] md:px-[20px] flex items-center justify-start">
            ...
            <div class="relative w-full md:w-[217px]">
                ...
                <input type="search" name="search" id="search" placeholder="Search..." value="" class="search !outline-none border border-[#0000001A] h-[40px] w-full p-[10px] pl-[42px] bg-transparent text-[#000000] placeholder:text-[#6F6F6F] rounded-[10px] text-[14px] font-[400] leading-[16px] ">
            </div>
        </div>
        <div class="overflow-x-auto " id="search_table_data">
            <table width="100%" cellpadding="0" cellspacing="0" class="min-w-[700px]">
                ...
                <tbody>
                    ...
                    @if ($data->count())                    
                        @foreach ($data as $scopeOfBusiness)
                            <tr>
                                ...
                                {{-- <td class="border-b-[1px] border-[#0000001A] text-start text-[12px] md:text-[14px] whitespace-nowrap font-[400] leading-[16px] text-[#000000] py-[12px] px-[15px]">
                                    {{$scopeOfBusiness->mobile}}
                                </td> --}}
                                
                                <td class="border-b-[1px] border-[#0000001A] text-start text-[12px] md:text-[14px] whitespace-nowrap font-[400] leading-[16px] text-[#000000] py-[12px] px-[15px]">
                                    @if ($scopeOfBusiness->status == 1)
                                        ...
                                        <div class="dropdown_menus absolute right-0 z-10 mt-2 w-[100px] origin-top-right rounded-md bg-white shadow-md ring-1 ring-black/5 focus:outline-none hidden" role="menu" aria-orientation="vertical" aria-labelledby="menu-button" tabindex="-1">
                                            <div class="dropdown_menus absolute right-0 z-10 mt-2 w-[100px] origin-top-right rounded-md bg-white shadow-md ring-1 ring-black/5 focus:outline-none hidden" role="menu" aria-orientation="vertical" aria-labelledby="menu-button" tabindex="-1">
                                                <div class="text-start" role="none">
                                                    @if(in_array('setting.scopeOfBusinessAdd',$permissionDetails['accessableRoutes']) || auth()->user()->role==1)
                                                    <a href="javascript:void(0)"
                                                    type="button"
                                                    data-name="{{$scopeOfBusiness->title}}"
                                                    data-id="{{$scopeOfBusiness->id}}"
                                                    class="openModalEdit block border-b-[1px] border-[#0000001A] hover:bg-[#f7f7f7] px-3 py-1 text-[12px] text-gray-700"
                                                    data-modal-target="assignUserEditModal"
                                                    data-modal-toggle="assignUserEditModal">
                                                    Edit
                                                 </a>
                                                    @endif
                                                    @if(in_array('setting.scopeOfBusinessStatus',$permissionDetails['accessableRoutes']) || auth()->user()->role==1)
                                                        <a href="{{ route('setting.scopeOfBusinessStatus',['id'=>$scopeOfBusiness->id])}}" class="client_status block border-b-[1px] border-[#0000001A] hover:bg-[#f7f7f7] px-3 py-1 text-[12px] text-gray-700">
                                                        {{ $scopeOfBusiness->status ? 'Inactive' : 'Active' }} </a>
                                                    @endif
                    ...
                    @else
                        <tr>
                            <td colspan="5" class="mt-5 text-center text-red-500">No Record Found!</td>
                        </tr>
                    @endif
            ...
            <div class="py-[15px] px-[20px]">
                {{ $data->appends(['key' => $searchKey])->links() }}
            </div>
        </div>
...
<div id="assignUserModal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[100%)] max-h-full bg-[rgba(0,0,0,0.6)] ">
    <div class="relative p-4 w-full max-w-[780px] max-h-full m-auto">
        ...
        <div class="relative bg-white rounded-[20px] shadow dark:bg-gray-700">
            ...
            <!-- Modal body -->
            <div class="p-[20px]">
                <form method="POST" action={{ route('setting.scopeOfBusinessAdd')}} class="space-y-[20px]">
                    @csrf
...
<div id="assignUserEditModal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[100%)] max-h-full bg-[rgba(0,0,0,0.6)] ">
    <div class="relative p-4 w-full max-w-[780px] max-h-full m-auto">
        <!-- Modal content -->
        <div class="relative bg-white rounded-[20px] shadow dark:bg-gray-700">
            <!-- Modal header -->
            <div class="flex items-center justify-between p-4 md:px-5 md:py-[20px] border-b border-[#0000001A] rounded-t dark:border-gray-600">
                ...
                <button type="button" data-modal-hide="assignUserEditModal" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="assignUserModal">
                    <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                        <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6" />
                    </svg>
                    <span class="sr-only">Close modal</span>
            ...
            <!-- Modal body -->
            <div class="p-[20px]">
                <form method="POST" action={{ route('setting.scopeOfBusinessUpdate')}} class="space-y-[20px]">
                    @csrf
                    <input type="hidden" name="type" id="type" value="2">
                    <input type="hidden" name="scope_of_business_id" id="scope_of_business_id" value="">
                    <div class="flex flex-col md:flex-row gap-[20px]">
                        
                        <div class="w-full">
                            <label for="name" class="block text-[14px] font-[400] leading-[16px] text-[#000000] mb-[5px]">Name</label>
                            <input type="text" name="scope_of_business_name" id="scope_of_business_name" value="" class="w-full h-[45px] border-[1px] border-[#0000001A] text-[14px] font-[400] leading-[16px] text-[#000000] tracking-[0.01em] px-[15px] py-[10px] rounded-[10px] !outline-none">
                        </div>
                    </div>
                   
                    <div class="flex justify-end gap-[15px]">                        
                        <button type="submit" data-id="" data-type="" data-name="" class="text-[13px] font-[500] leading-[15px] text-[#ffffff] tracking-[   ] bg-[#13103A] rounded-[10px] py-[12px] px-[30px]">Save</button>
                    </div>
                </form>
            </div>
        </div>
...
<script>
     $(document).on('click','.openModalScope',function(){
    ...
    let id = $(this).data('id');
    $('#scope_of_business_id').val(id);
    $('#scope_of_business_name').val(name);
    
    $('#assignUserEditModal').removeClass('hidden').addClass('flex');
    $(document).on('click', '[data-modal-hide="assignUserModal"]', function () {
    $('#assignUserEditModal').removeClass('flex').addClass('hidden');
    });
...
$(document).on('keyup', '.search', function() {
        ...
        $.ajax({
            method: 'POST',
            url: "{{ route('setting.scopeOfBusiness')}}",
            headers: {
