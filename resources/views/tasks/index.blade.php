    ...
    <div>
        <div class="shadow-[0px_0px_13px_5px_#0000000f] bg-white rounded-[20px] overflow-hidden ">
            <div class="py-[15px] md:py-[25px] px-[15px] md:px-[20px] gap-[10px] flex flex-col md:flex-row items-center justify-between">
                <div class="flex max-w-[600px] gap-[10px] w-full items-center justify-start">
                    {{-- <label for="client-name" class="text-[14px] font-[400] text-[#13103A]">Client Name</label>
                    <select name="sorting" id="client-name" class="allform-select2 !outline-none h-[40px] border border-[#0000001A] w-full md:w-[95px] rounded-[10px] p-[10px] text-[14px] font-[400] leading-[16px] text-[#13103A]">
                    ...
                    <select name="status" id="status" class="allform-select2 !outline-none h-[40px] border border-[#0000001A] w-full md:w-[90px] rounded-[10px] p-[10px] text-[14px] font-[400] leading-[16px] text-[#13103A]">
                        ...
                        @php
                            $status = 'other';
                            if($details->leadTaskDetails->status == 0){
                            $status = 'Pending';
                            }else if($details->leadTaskDetails->status == 1){
                            $status = 'Completed';
                            }else if($details->leadTaskDetails->status == 2){
                            $status = 'Hold';
                            }else if($details->leadTaskDetails->status == 3){
                            $status = 'follow-up';
                            }
                        @endphp
                            <option value="{{ $details->leadTaskDetails->id }}">{{ $status }}</option>
                        @endforeach
                        @else
