...
<div class="shadow-[0px_0px_13px_5px_#0000000f] bg-white px-[15px] md:px-[30px] py-[20px] rounded-[20px] mt-[20px] overflow-hidden ">
   <form action="{{route('task.patentCounterStatementHearingSubmit',['id'=>$id]) }}" method="POST" class="space-y-[20px]" enctype="multipart/form-data">
      @csrf
      <strong class="mt-4 block"> Update Current Task</strong>

      <div class="flex flex-col md:flex-row gap-[20px]">
        <input type="hidden" name="old_count_id" id="old_count_id" value="{{$taskDetails->service_stage_id}}">
         ...
         <div class="w-full md:w-1/2" id="verifiedDate">
            <label for="verified" class="dateChangeText block text-[14px] font-[400] leading-[16px] text-[#000000] mb-[5px]">
               Verified On
            </label>
      ...
      <div class="flex flex-col md:flex-row gap-[20px]">
         ...
         <div class="w-full md:w-1/2 hearingDate hidden">
            ...
            <p class="hearingDate hidden" style="color: skyblue; font-size: 14px; font-weight: 500;">
               This will be {{ $count }}{{ $count == 1 ? 'st' : ($count == 2 ? 'nd' : ($count == 3 ? 'rd' : 'th')) }} Hearing
             </p>
         </div>
         @error('hearing_date')
         <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
      ...
      <div class=" onHide flex flex-col md:flex-row gap-[20px]">
         <div class="w-full md:w-1/2 ">
            ...
            @if($getStage != null && $getStage->count() > 0)
            ...
            <input type="hidden" name="stage_id" id="StageID" value="{{$getStage->id}}">
            @endif
...
$('.daterangepicker-taskdeadline').attr("placeholder", "DD/MM/YYYY"); 
 ...
 $('.daterangepicker-taskdeadline').daterangepicker({
    ...
    console.log("A new date selection was made: " + picker.startDate.format('YYYY-MM-DD'));
      ...
      $("#show_case_hearing").on('change', function() {
         var show_case_hearing = $(this).val();

         if(show_case_hearing == 1){
