...
<div class="shadow-[0px_0px_13px_5px_#0000000f] bg-white px-[15px] md:px-[30px] py-[20px] rounded-[20px] mt-[20px] overflow-hidden ">
   <form action="{{route('task.patentfilingProccessStatus',['id'=>$id]) }}" method="POST" class="space-y-[20px]" enctype="multipart/form-data">
      @csrf
      ...
      <div class="flex flex-col md:flex-row gap-[20px]">
         <div class="w-full md:w-1/2">
            ...
            <input type="text" name="application_number" id="application_number" value="" placeholder="Enter Application Number" class="w-full h-[45px] border-[1px] border-[#0000001A] text-[14px] font-[400] leading-[16px] text-[#000000] tracking-[0.01em] px-[15px] py-[10px] rounded-[10px] !outline-none">
   ...
   <table width="600" border="0" cellspacing="0" cellpadding="0" style="background-color: #ffffff; border-radius: 12px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.15); overflow: hidden;">
      <tr>
         <td style="background: url('assets/images/login-bg.jpg') center/cover no-repeat; padding: 10px 10px 0; text-align: center;">
            <img src="{{asset('assets/images/logo.png')}}" alt="Your Logo" style="max-width: 200px; margin-bottom: 10px; margin:auto;">
      ...
      <tr>
         <td style="padding: 15px 15px; text-align: left;">
            <p style="font-size: 16px; line-height: 20px; margin: 0 0 5px; color: #555;">
              Hello <strong>{{ $taskDetails->lead->client_name }}</strong> 
            </p>
            <p style="font-size: 14px; line-height: 20px; margin: 0 0 10px; color: #555;">
               
               Thank you for confirming your details and submitting the required documents. We appreciate your cooperation.

               We have successfully verified the submitted information on the portal. Based on this confirmation, we will now proceed with the Formality Examination stage of your patent application. Please find the details below for your reference:
            </p>
            ...
            <div id="modal-file-preview" class="flex gap-4 flex-wrap"></div>

         </div>
           
            <p style="font-size: 15px; text-align: center; line-height: 20px; margin: 15px 0 0; color: #555;">
