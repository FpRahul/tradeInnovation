   ...
   <form action="{{ route('')}}" method="POST" class="space-y-[20px]" enctype="multipart/form-data">
      ...
      <div class="flex flex-col md:flex-row gap-[20px]">
         <div class="flex justify-start flex-wrap  w-[100%] md:w-[49%]">
            <label class="w-full flex  text-[14px] font-[400] leading-[16px] text-[#d51111] mb-[5px]">You can choose multiple files</label>
            <label for="attachment" class="attachementbx flex items-center gap-[10px] w-full text-[13px] font-[500] leading-[15px] text-[#666666] tracking-[0.01em] bg-[#fff] border-dashed border-[1px] border-[#ccc] rounded-[6px] py-[6px] px-[5px] cursor-pointer">
               ...
               <input type="file" id="attachment" name="attachment[]" class="block-none previewImage opacity-0" multiple />
            </label>
            
            <div id="fileNames" class="fileOutput"></div>
...
<script>
   ...
   $(document).on('change', '.previewImage', function() {
      ...
      let fileNamesContainer = $('#fileNames');
      fileNamesContainer.empty(); // Clear previous entries
