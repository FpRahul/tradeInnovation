...
<div class="shadow-[0px_0px_13px_5px_#0000000f] bg-white px-[15px] md:px-[30px] py-[20px] rounded-[20px] mt-[20px] overflow-hidden ">
   <strong>Update Current Task</strong>
    <form action="{{route('task.patentSubmitPriorArt',['id'=>$taskId]) }}" method="POST" class="space-y-[20px]" enctype="multipart/form-data">
        @csrf
        ...
        <div class="flex flex-wrap gap-[20px]">    
            <div class="w-full lg:w-[49%]">
                <label for="relevantpriorart" class="block text-[14px] font-[400] leading-[16px] text-[#000000] mb-[5px]">Relevant Prior Art</label>
                <select name="relevantpriorart" id="relevantpriorart" class="relevantPriorArtData allform-select2 !outline-none h-[45px] border border-[#0000001A] w-full md:w-[95px] rounded-[10px] p-[10px] text-[14px] font-[400] leading-[16px] text-[#13103A]" required>
                    <option value="" disabled selected>Select a user</option>  
                    <option value="0">Relevant prior art found</option>
                    <option value="1">No relevant prior art found</option>             
                </select>                
            </div>  
            <div class="w-full lg:w-[49%]" id="verifiedDate">
            ...
            <div class="flex justify-start flex-wrap w-full lg:w-[49%]">
                ...
                <div id="file-list" class="mt-2 flex gap-[5px] flex-wrap"></div>
            </div>
            @error('attachment.*')
        ...
        <div class="noRelevantFound flex flex-wrap gap-[20px] hidden">   
                ...
                <div class="w-full md:w-[48%]" id="deadlineDate">
                    ...
                    <input
                        ...
                        placeholder="Dead Line"
                        name="deadline"
                        class="daterangepicker-deadline w-[100%] h-[45px] border-[1px] border-[#0000001A] text-[14px] font-[400] leading-[16px] text-[#000000] tracking-[0.01em] px-[15px] py-[10px] rounded-[10px] outline-none bg-transparent z-10 relative"
                        value=""
                        id="deadline"
                        autocomplete="off">
                    <div class="absolute right-[10px] top-[10px] z-0">
        ...
        <div class="flex justify-end gap-[15px]">
            <button type="submit" class="text-[13px] font-[500] leading-[15px] text-[#ffffff] tracking-[0.01em] bg-[#13103A] rounded-[10px] py-[12px] px-[30px]">Submit</button>
        </div>
    </form>
</div>
...
<script>
   $(document).ready(function() {
    ...
    $(document).on('change','.relevantPriorArtData',function(){
        let val = $(this).val();
        if(val == 1){
            $('.noRelevantFound').removeClass('hidden');
            $('.noRelevantFound').find('#assignUser').prop('required',true);
            $('.noRelevantFound').find('#stage_id').prop('required',true);
            $('.noRelevantFound').find('#deadline').prop('required',true);
        }else{
            $('.noRelevantFound').addClass('hidden');
            $('.noRelevantFound').find('#assignUser').prop('required',false);
            $('.noRelevantFound').find('#stage_id').prop('required',false);
            $('.noRelevantFound').find('#deadline').prop('required',false);
        }
    });
</script>
@stop