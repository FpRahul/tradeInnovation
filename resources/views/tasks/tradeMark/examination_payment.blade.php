...
<div class="shadow-[0px_0px_13px_5px_#0000000f] bg-white px-[15px] md:px-[30px] py-[20px] rounded-[20px] mt-[20px] overflow-hidden ">
   <form action="{{route('task.examinationPaymentStatus',['id'=>$id]) }}" method="POST" class="space-y-[20px]" enctype="multipart/form-data">
      @csrf
       ...
       <div class="relative bg-white rounded-[20px] shadow dark:bg-gray-700">
           ...
           <div class="p-[20px]">
               <form method="POST" action={{ route('task.negotiatePrice',['id'=>$taskDetailsId])}} class="space-y-[20px]">
                   @csrf
                   <span>Original Service Price {{$payamentDetails->service_price}}</span>
                  <div class="flex flex-col md:flex-row gap-[20px]">                     
                     <div class="w-full">
