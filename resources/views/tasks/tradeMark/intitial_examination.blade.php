...
<div class="shadow-[0px_0px_13px_5px_#0000000f] bg-white px-[15px] md:px-[30px] py-[20px] rounded-[20px] mt-[20px] overflow-hidden ">
   <form action="{{route('task.initialExaminationStatus',['id'=>$id]) }}" method="POST" class="space-y-[20px]" enctype="multipart/form-data">
      @csrf
      ...
      <div class="flex flex-col md:flex-row gap-[20px]">
         ...
         <div class="w-full md:w-1/2">
            <label for="initial_examination" class="block text-[14px] font-[400] leading-[16px] text-[#000000] mb-[5px]">Status</label>
            <select name="initial_examination" id="initial_examination" class="w-full h-[45px] border-[1px] border-[#0000001A] text-[14px] font-[400] leading-[16px] text-[#000000] tracking-[0.01em] px-[15px] py-[10px] rounded-[10px] !outline-none">
               <option value="" disabled selected>Select status</option>
               <option value="0"> Accepeted</option>
               <option value="1"> Objected </option>
            </select>
            <div class="showWarning" style="color: red;font-size: 14px; font-weight: 500;"></div>
            @error('initial_examination')
    ...
    console.log("A new date selection was made: " + picker.startDate.format('YYYY-MM-DD'));
      ...
      $("#initial_examination").on('change', function () {
         ...
         if (changeValue == 0) {
            ...
            $("#verifiedDate label").text("Objection Date");
         }
