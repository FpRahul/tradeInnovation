...
<div class="shadow-[0px_0px_13px_5px_#0000000f] bg-white px-[15px] md:px-[30px] py-[20px] rounded-[20px] mt-[20px] overflow-hidden ">
   <form action="{{route('task.govtPortalReplyStatus',['id'=>$id]) }}" method="POST" class="space-y-[20px]" enctype="multipart/form-data">
      @csrf
      ...
      <div class="flex flex-col md:flex-row gap-[20px]">
         ...
         <div class="w-full md:w-1/2">
            <label for="reply_status" class="block text-[14px] font-[400] leading-[16px] text-[#000000] mb-[5px]">Status</label>
            <select name="reply_status" id="reply_status" class="w-full h-[45px] border-[1px] border-[#0000001A] text-[14px] font-[400] leading-[16px] text-[#000000] tracking-[0.01em] px-[15px] py-[10px] rounded-[10px] !outline-none">
               <option value="" disabled selected>Select status</option>
               <option value="0"> Accepeted</option>
               <option value="1"> Ready For Show Cause Hearing  </option>
            </select>
...
<script>
   $(document).ready(function() {
      ...
      $("#reply_status").on('change', function() {
         var reply_status = $(this).val();
         if (reply_status == 1) {
