...
<div class="shadow-[0px_0px_13px_5px_#0000000f] bg-white px-[15px] md:px-[30px] py-[20px] rounded-[20px] mt-[20px] overflow-hidden ">
   ...
   <form action="{{route('task.sendQuotation',['id'=>$id]) }}" method="POST" class="space-y-[20px]" enctype="multipart/form-data">
      ...
      <div class="w-full md:w-1/2" id="verifiedDate">
         ...
         <p class="infoUser" style="color: skyblue; font-size: 14px; font-weight: 500;">Set A Dead Line For:{{ $upcomeing->title }}
         </p>
      </div>
      ...
      <div class="flex flex-col md:flex-row gap-[20px]">
         ...
         <div class="w-full md:w-1/2">
            ...
            <input type="text" name="govt_price" id="govt_price" value="" placeholder="Enter service govt price" class="w-full h-[45px] border-[1px] border-[#0000001A] text-[14px] font-[400] leading-[16px] text-[#000000] tracking-[0.01em] px-[15px] py-[10px] rounded-[10px] !outline-none" required>
            @error('govt_price')
            <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
            @enderror
         </div>
