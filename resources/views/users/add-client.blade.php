        ...
        <form method="POST" action="{{ route('users.addclient',['id' => $newClient->id])}}" class="py-[15px] md:py-[25px] px-[15px] md:px-[30px] space-y-[20px]">
            ...
            <div class="flex flex-col md:flex-row gap-[20px] !mt-0">
                ...
                <div class="w-full md:w-1/2">
                    ...
                    <input type="text" name="lastName" id="last_name" value="{{ ucwords(old('last_name') ? old('last_name') : $newClient->last_name)}}" class="w-full h-[45px] border-[1px] border-[#0000001A] text-[14px] font-[400] leading-[16px] text-[#000000] tracking-[0.01em] px-[15px] py-[10px] rounded-[10px] !outline-none" placeholder="Enter Name" required>
                    @error('last_name')
                    <div class="alert alert-danger">{{ $message }}</div>
                    @enderror
                </div>
