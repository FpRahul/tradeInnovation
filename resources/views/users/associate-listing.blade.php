    ...
    <div class="shadow-[0px_0px_13px_5px_#0000000f] bg-white rounded-[20px] ">
        <div class="py-[15px] md:py-[25px] px-[15px] md:px-[20px] flex items-center justify-end">
            ...
            <div class="relative w-full md:w-[217px]">
                ...
                <input type="search" name="search" id="search" placeholder="Search..." value="{{$searchKey}}" class="search !outline-none border border-[#0000001A] h-[40px] w-full p-[10px] pl-[42px] bg-transparent text-[#000000] placeholder:text-[#6F6F6F] rounded-[10px] text-[14px] font-[400] leading-[16px] ">
            </div>
        </div>
                    ...
                    @if (count($associateData) > 0)
                        ...
                        <tr>
                            ...
                            <td class="border-b-[1px] border-[#0000001A] py-[12px] px-[15px]">
                                @if((in_array('users.addclient',$permissionDetails['accessableRoutes']) || in_array('associate.status',$permissionDetails['accessableRoutes']) || in_array('users.delete',$permissionDetails['accessableRoutes'])) || auth()->user()->role==1)
                                <div class="dropdown inline-block relative ml-[auto] mr-[20px] ">
                                    ...
                                    <div class="dropdown_menus absolute right-0 z-10 mt-2 w-[100px] origin-top-right rounded-md bg-white shadow-md ring-1 ring-black/5 focus:outline-none hidden" role="menu" aria-orientation="vertical" aria-labelledby="menu-button" tabindex="-1">
                                        <div class="text-start" role="none">
                                            ...
                                            @endif --}}
                                            @if(in_array('associate.status',$permissionDetails['accessableRoutes']) || auth()->user()->role==1)
                                                <a href="{{ route('associate.status', ['id' => $associateDetails->id, 'val' => $associateDetails->status]) }}" class="associate_status block border-b-[1px] border-[#0000001A] hover:bg-[#f7f7f7] px-3 py-1 text-[12px] text-gray-700">
