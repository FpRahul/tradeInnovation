...
<div>
    <div class="flex items-center justify-between mb-[20px]">
        <div>
            <h3 class="text-[20px] font-[500] leading-[24px] text-[#13103A] tracking-[0.02em] mb-1.5">Manage Incorporation</h3>
            <ul class="flex items-center text-[14px] font-[400] leading-[16px] text-[#000000] gap-[5px]">
                <li>{{$header_title_name}}</li> /
                <li class="text-gray">Incorporation</li>
            </ul>
        </div>
                        ...
                        @foreach ($categoryData as $newCategorylist)
                            ...
                            <tr>
                                ...
                                <td class="border-b-[1px] border-[#0000001A] py-[12px] px-[15px]">
                                    @if((in_array('incorporations.add',$permissionDetails['accessableRoutes']) || in_array('incorporations.status',$permissionDetails['accessableRoutes'])) || auth()->user()->role==1)
                                    <div class="dropdown inline-block relative ml-[auto] mr-[20px] ">
                                        ...
                                        <div class="dropdown_menus absolute right-0 z-10 mt-2 w-[100px] origin-top-right rounded-md bg-white shadow-md ring-1 ring-black/5 focus:outline-none hidden" role="menu" aria-orientation="vertical" aria-labelledby="menu-button" tabindex="-1">
                                            <div class="text-start" role="none">
                                                @if(in_array('incorporations.add',$permissionDetails['accessableRoutes']) || auth()->user()->role==1)
                                                    <a href="javascript:void(0)" type="button" data-btn-name="edit" data-name="{{$newCategorylist->name}}" data-id="{{$newCategorylist->id}}" class="openModalIncor block border-b-[1px] border-[#0000001A] hover:bg-[#f7f7f7] px-3 py-1 text-[12px] text-gray-700" data-modal-target="assignUserModal" data-modal-toggle="assignUserModal">Edit</a>
                                                @endif
                                                @if(in_array('incorporations.status',$permissionDetails['accessableRoutes']) || auth()->user()->role==1)
                                                    <a href="{{ route('incorporations.status',['id'=>$newCategorylist->id,'val' => $newCategorylist->status])}}" class="client_status block border-b-[1px] border-[#0000001A] hover:bg-[#f7f7f7] px-3 py-1 text-[12px] text-gray-700">
