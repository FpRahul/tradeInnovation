...
@section('content')
<div class="bg-cover bg-center bg-no-repeat bg-fixed" style="background-image: asset('assets/images/login-bg.jpg');">
    <div class="max-w-[1200px] py-[30px] px-[15px] mx-auto w-full min-h-[100vh] flex flex-col items-center justify-between">
        ...
        <img src="{{asset('assets/images/logo.png')}}" alt="logo" class="mb-[23px] mx-auto ">
            ...
            <h2 class="font-[Inter] text-[20px] font-[500] text-[#13103A] leading-[24px] tracking-[-0.02em] mb-2 ">Reset Password</h2>
            <p class="font-[Inter] text-[14px] font-[400] text-[#13103A] leading-[16px]">Secure Access to Intellectual Property Management</p>

            <form class="mt-[20px] space-y-[15px]" method="POST" action="{{ route('passwordReset.passwordReset', ['token' => $token]) }}">
                @csrf
                <div>
                    <label for="password" class="block font-[Inter] text-[14px] font-[400] text-[#13103A] leading-[16px] mb-1">New Password</label>
                    <div class="relative">
                        <input type="password" name="newPassword" id="newPassword" class="font-[Inter] text-[14px] font-[400] text-[#6F6F6F] leading-[16px] border-[1px] border-[#0000001A] rounded-[10px] h-[45px] px-[15px] p-[10px] w-full">
                        @error('newPassword')
                ...
                <div>
                    <label for="password" class="block font-[Inter] text-[14px] font-[400] text-[#13103A] leading-[16px] mb-1">Confirm Password</label>
                    <div class="relative">
                        <input type="password" name="confirmPassword" id="confirmPassword" class="font-[Inter] text-[14px] font-[400] text-[#6F6F6F] leading-[16px] border-[1px] border-[#0000001A] rounded-[10px] h-[45px] px-[15px] p-[10px] w-full">
                        @error('confirmPassword')
                        <div class="alert alert-danger">{{ $message }}</div>
                        @enderror
                        <a id="eyeSvg" class="absolute top-[50%] right-[15px] translate-y-[-50%] bg-transparent border-none ">
                            <svg class="svgEye" width="16" height="14" viewBox="0 0 16 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M2.66672 1.66675L13.3334 12.3334M11.0001 10.1707C10.0981 10.6567 9.07872 11.0001 8.00005 11.0001C5.64672 11.0001 3.57739 9.36542 2.39139 8.18608C2.07805 7.87475 1.92139 7.71941 1.82205 7.41341C1.75072 7.19541 1.75072 6.80475 1.82205 6.58675C1.92205 6.28075 2.07872 6.12475 2.39205 5.81342C2.99005 5.21875 3.81205 4.50941 4.78139 3.95141M13.0001 8.75608C13.2221 8.56075 13.4254 8.36808 13.6081 8.18675L13.6101 8.18475C13.9221 7.87408 14.0787 7.71808 14.1781 7.41408C14.2494 7.19608 14.2494 6.80475 14.1781 6.58675C14.0781 6.28141 13.9221 6.12541 13.6087 5.81408C12.4221 4.63475 10.3541 3.00008 8.00005 3.00008C7.77472 3.00008 7.5525 3.0143 7.33339 3.04275M8.88205 8.00008C8.62383 8.2281 8.28737 8.34733 7.94319 8.33278C7.59901 8.31823 7.27381 8.17103 7.03576 7.92202C6.7977 7.67302 6.66526 7.34154 6.66618 6.99705C6.66711 6.65256 6.80133 6.3218 7.04072 6.07408" stroke="#6F6F6F" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                            </svg>
                        </a>
                    </div>
                ...
                {{-- <div>
                    <a href="{{ route('forgetPassword')}}" class="font-[Inter] text-[14px] font-[400] text-[#13103A] leading-[16px]"></a>
                </div> --}}
                <div>
        ...
        $('#eyeBtn').on('click', function() {
            ...
            if (newPassword.attr('type') === 'password') {
               newPassword.attr('type', 'text'); 
                eyeIcon.css('stroke', '#000');
            } else {
               newPassword.attr('type', 'password');
                eyeIcon.css('stroke', '#6F6F6F'); 
            }

            
        });
        $('#eyeSvg').on('click', function() {
